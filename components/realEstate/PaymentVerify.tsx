...
export default function PaymentVerifyDialog(props: Props) {
  ...
  const handleApprove = async () => {
    await apiGetRequest('/payments/estate/bank/confirm/' + paymentId, currentUser?.accessToken);
    handleClose();
  }
  ...
  const handleSubmitReject = async () => {
    if (remarks.length == 0) setError(true);
      ...
      await apiPostRequest('/payments/estate/reject', body, currentUser?.accessToken);
      handleClose();
    }
  }
